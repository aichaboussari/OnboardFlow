...
import { getChecklists, updateChecklist } from '../../../lib/checklists';

export const Route = createFileRoute('/app/checklists/$id/edit')({
  component: EditChecklistComponent,
});

function EditChecklistComponent() {
  const { id } = useParams({ from: '/app/checklists/$id/edit' });
  ...
  useEffect(() => {
    const fetchChecklist = async () => {
      ...
      if (!sessionId) return;
      const checklists = await getChecklists({ data: { sessionId } });
      const checklist = checklists.find((c: any) => c.id === id);
      if (checklist) {
        setName(checklist.name);
        setTasks(checklist.tasks.map((t: any) => t.task_name));
      } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await updateChecklist({ data: { sessionId, id: id as string, name, tasks: validTasks } });
      if (result.success) {
  ...
  if (loading) return <div className="text-center py-10">Loading checklist...</div>;

  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Edit Task Checklist</h2>
      </div>
